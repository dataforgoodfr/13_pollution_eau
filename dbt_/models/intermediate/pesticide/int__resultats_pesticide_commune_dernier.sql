...
aggregated AS (
    ...
    FROM last_pvl
    WHERE row_number = 1
    GROUP BY inseecommune, cdparametresiseeaux, categorie_2, categorie_3
),
        ...
        *,
        SUM(
            CASE
                ...
                ELSE 0
            END
        ) OVER (PARTITION BY inseecommune) AS total_pesticide,
        SUM(
            CASE
                WHEN
                    (
                ...
                ELSE 0
            END
        ) OVER (PARTITION BY inseecommune) AS total_pesticide_all
    FROM aggregated
)
