...
-- Population par catégorie et résultat pour dernier prélèvement
stats_dernier_prel_population AS (
    SELECT
        NULL AS stat_texte,
        'dernier_prel_' || categorie || '_' || coalesce(resultat, 'non_recherche') || '_population'
            AS stat_nom,
        sum(population) AS stat_chiffre
    FROM {{ ref('web__resultats_udi') }}
    WHERE
        periode = 'dernier_prel'
...
-- Population par catégorie et année pour bilan annuel - ratios par intervalles
stats_bilan_annuel_ratio_population AS (
    SELECT
        NULL AS stat_texte,
        periode || '_' || categorie || '_ratio_'
        || CASE
...
-- Population par catégorie et année pour bilan annuel - non recherche (ratio null)
stats_bilan_annuel_non_recherche_population AS (
    SELECT
        NULL AS stat_texte,
        periode || '_' || categorie || '_non_recherche_population' AS stat_nom,
        sum(population) AS stat_chiffre
    FROM {{ ref('web__resultats_udi') }}
    WHERE
        periode LIKE 'bilan_annuel_%'
...
SELECT
    ...
    stat_chiffre,
    stat_texte
FROM stats_dernier_prel_population
UNION ALL
SELECT
    stat_nom,
    stat_chiffre,
...
SELECT
    ...
    stat_chiffre,
    stat_texte
FROM stats_bilan_annuel_ratio_population
UNION ALL
SELECT
    stat_nom,
    stat_chiffre,
    stat_texte
FROM stats_bilan_annuel_non_recherche_population
