...
class ObjectStorageClient:
    ...
    def list_objects(self, prefix=None):
        try:
            response = self.client_v4.list_objects(Bucket=self.bucket_name, Prefix=prefix)
            if "Contents" in response:
                return response["Contents"]
            else:
                return []
        except ClientError as e:
            logger.error("Error list_objects in bucket %s: %s/%s", self.bucket_name, prefix, e)
    ...
    def upload_object(self, local_path, file_key=None, public_read=False):
        try:
            ...
            logger.error("Boto Client Error upload_object '%s' to bucket '%s': %s", local_path, self.bucket_name, e)
