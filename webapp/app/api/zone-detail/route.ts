...
const NUMERIC_FIELDS = ["ratio", "nb_parametres", "nb_prelevements", "nb_sup_valeur_sanitaire"];
...
async function fetchZoneDetail(
  ...
  const table = type === "udis" ? "web__resultats_udi" : "web__resultats_communes";
  ...
  try {
    ...
    if (type === "udis") {
      data["nomreseaux"] = rows[0].nomreseaux ? String(rows[0].nomreseaux) : null;
      data["population"] = rows[0].population ? Number(rows[0].population) : null;
    } else {
      data["commune_nom"] = rows[0].commune_nom ? String(rows[0].commune_nom) : null;
    }
